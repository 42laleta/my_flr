...
void				ft_draw_header(t_sfml *sfml)
{
	...
	sfSprite_setTextureRect(sfml->p1->sprite_s, irect);
	ft_draw_header_sprite(sfml, sfml->p1->sprite_s, MARGIN, 280);
	irect.left = 0;
	irect.top = 0;
	irect.height = CELL_SZ;
	irect.width = (double)sfml->p2->score / sfml->cell_cnt * sfml->width_sc;
	sfSprite_setTextureRect(sfml->p2->sprite_s, irect);
	ft_draw_header_sprite(sfml, sfml->p2->sprite_s, MARGIN, 400);
}
